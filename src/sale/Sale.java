...
public class Sale implements Comparable<Sale> {
	...
	/**
	 * Method to print out only basic sale info to a string.
	 */
	public static String saleString(AdtList<Sale> sales, String sortType) {
		StringBuilder sb = new StringBuilder();
		sb.append("+---------------------------------------------------------------------------------------------+\n");
		sb.append("| Sales by                           			              |\n");
		sb.append(sortType + "\n");
		sb.append("+---------------------------------------------------------------------------------------------+\n");
		sb.append("Sale\tStore\tCustomer\t\tSalesperson\t\tTotal\n");
		for(Sale s : sales) {
			sb.append(s.getSaleCode() + "\t" + s.getStore().getStoreCode() + "\t" + 
					s.getCustomer().personNameToString() + "\t\t" + 
					s.getSalesPerson().personNameToString() + "\t\t" + 
					Double.toString(s.getGrandTotal()) + "\n");
		}
		return sb.toString();
	}
	...
	/**
	 * GUI use: Method to print out list of sale info to a string.
	 */
	public static String stringListSale(AdtList<Sale> sales) {
		StringBuilder sb = new StringBuilder();
		for(Sale s : sales) {
			...
			double taxAmount = 0.0;
			for(Item listItem : salesItem) {
				sb.append(String.format("%s%-60s", listItem.nameToString(), listItem.saleInfoToString()));
			...
			sb.append(String.format("%60s $ %9.2f\n", "Subtotal", subTotal));
			sb.append(String.format("%60s $ %9.2f\n", "Tax", taxAmount));
			if(p.getDiscount() == .15) {
				sb.append(String.format("%51s (%.2f%%) $ %9.2f\n", "Discount", (p.getDiscount() * 100), discount));
			}else if (p.getDiscount() == .10) {
				sb.append(String.format("%51s (%.2f%%) $ %9.2f\n", "Discount", (p.getDiscount() * 100), discount));
			} else if (p.getDiscount() == .05) {
				sb.append(String.format("%52s (%.2f%%) $ %9.2f\n", "Discount", (p.getDiscount() * 100), discount));
			} else if (p.getDiscount() == 0.0) {
				//Do not print discount
			}
			sb.append(String.format("%60s $ %9.2f\n", "Grand Total", grandTotal));
